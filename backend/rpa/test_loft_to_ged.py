#!/usr/bin/env python
"""
Script de teste end-to-end: BCLegal Loft → ADVWin GED

Este script:
1. Baixa documentos do BCLegal Loft para um contrato específico
...
downloads_dir = os.path.join(BASE_DIR, "downloads_teste_loft")
os.makedirs(downloads_dir, exist_ok=True)

log_filename = os.path.join(
    log_dir, f"test_loft_to_ged_{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}.log"
)
...
# Número do contrato/processo para teste - AJUSTE CONFORME NECESSÁRIO
NUMERO_CONTRATO_TESTE = "670122"  # ← ALTERE AQUI
...
def test_loft_download():
    """
    Etapa 1: Baixa documentos do BCLegal Loft
    ...
    logger.info("")
    logger.info("#" * 80)
    logger.info("# ETAPA 1: DOWNLOAD DE DOCUMENTOS - BCLEGAL LOFT")
    logger.info("#" * 80)
    logger.info(f"# Contrato: {NUMERO_CONTRATO_TESTE}")
    logger.info("#" * 80)
    logger.info("")
    ...
    try:
        ...
        # Cria instância do BCLegal Loft
        logger.info("Inicializando BCLegal Loft...")
        loft = BCLegalLoft(download_path=downloads_dir)

        # Inicializa o driver
        logger.info("Inicializando driver...")
        if not loft.inicializar_driver(driver):
            logger.error("✗ FALHA - Erro ao inicializar driver!")
            return None

        # Acessa o portal
        logger.info("Acessando o portal BCLegal...")
        if not loft.acessar_portal():
            logger.error("✗ FALHA - Erro ao acessar portal!")
            return None

        logger.info("✓ Portal acessado")

        # Faz login
        logger.info("Realizando login no BCLegal Loft...")
        if not loft.fazer_login():
        ...
        logger.info(f"Baixando documentos do contrato {NUMERO_CONTRATO_TESTE}...")
        documentos = loft.baixa_documento_anexo(NUMERO_CONTRATO_TESTE)
...
def test_ged_upload(documentos):
    ...
    Args:
        documentos: Lista de documentos baixados do Loft
    ...
    try:
        ...
        # - id_or = número do processo/contrato
        codigo_or = "00005-001400"  # ← AJUSTE: Código da pasta no ADVWin
        id_or = NUMERO_CONTRATO_TESTE  # ← Número do contrato
        ...
        print("")
        print(f"Contrato: {NUMERO_CONTRATO_TESTE}")
        ...
        resultado = ged_helper.enviar_documentos_ged(
            ...
            numero_processo=NUMERO_CONTRATO_TESTE,
            tabela_or=tabela_or,
            codigo_or=codigo_or,
            id_or=id_or
        )
...
def main():
    ...
    logger.info("╔" + "═" * 78 + "╗")
    logger.info("║" + " " * 78 + "║")
    logger.info("║" + "  TESTE END-TO-END: BCLEGAL LOFT → ADVWIN GED".center(78) + "║")
    logger.info("║" + " " * 78 + "║")
    logger.info("╚" + "═" * 78 + "╝")
    logger.info("")
    ...
    # ETAPA 1: Download do Loft
    documentos = test_loft_download()
