#!/usr/bin/env python
"""
Script de teste para verificar o login e baixa de documentos no sistema Lexxy SuperSim
Autor: AutoDev
Data: 2025
"""
...
# Pega o diretório do script atual (...\robos\lexxy)
...
log_filename = os.path.join(
    log_dir, f"test_lexxy_supersim_{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}.log"
)
...
logger = logging.getLogger("TEST_LEXXY_SUPERSIM")
...
def criar_driver_selenoid(session_name="Lexxy SuperSim - Teste Automatico"):
...
def test_login_e_baixa_documentos_local():
    """Testa o login e baixa de documentos no sistema Lexxy SuperSim usando Chrome local"""
    logger.info("="*70)
    logger.info("TESTE COMPLETO - SISTEMA LEXXY SUPERSIM (CHROME LOCAL)")
    logger.info("="*70)
    ...
    try:
        ...
        # Cria instancia do Lexxy SuperSim passando o driver e download path
        lexxy = LexxySuperSim(driver=driver, download_path=downloads_dir)

        logger.info("="*70)
        logger.info("ETAPA 1: Acessando o sistema Lexxy SuperSim...")
        logger.info("="*70)
        ...
        if lexxy.baixa_documento_anexo(NUMERO_PROCESSO_TESTE):
            logger.info("="*70)
...
def test_login_e_baixa_documentos_selenoid():
    """Testa o login e baixa de documentos no sistema Lexxy SuperSim usando Selenoid"""
    logger.info("="*70)
    logger.info("TESTE COMPLETO - SISTEMA LEXXY SUPERSIM (SELENOID)")
    logger.info("="*70)
    ...
    try:
        ...
        driver = criar_driver_selenoid("Lexxy SuperSim - Teste Automatico")

        # Cria instancia do Lexxy SuperSim passando o driver
        lexxy = LexxySuperSim(driver=driver, download_path=downloads_dir)

        logger.info("="*70)
        logger.info("ETAPA 1: Acessando o sistema Lexxy SuperSim via Selenoid...")
        logger.info("="*70)
...
def test_acesso_rapido():
    ...
    logger.info("="*70)
    logger.info("TESTE RAPIDO - LOGIN LEXXY SUPERSIM")
    logger.info("="*70)
...
if __name__ == "__main__":
    ...
    logger.info("")
    logger.info("#"*70)
    logger.info("# SUITE DE TESTES - LEXXY SUPERSIM RPA")
    logger.info("#"*70)
    logger.info("")
